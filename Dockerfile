...
# Perform the build
WORKDIR /go/src/git.code.oa.com/henrylwang/argo
COPY . .
...
COPY --from=argo-build /go/src/git.code.oa.com/henrylwang/argo/dist/argoexec-linux-amd64 /usr/local/bin/argoexec
ENTRYPOINT [ "argoexec" ]
...
COPY --from=argo-build /go/src/git.code.oa.com/henrylwang/argo/dist/workflow-controller-linux-amd64 /bin/workflow-controller
...
COPY --from=argo-build /go/src/git.code.oa.com/henrylwang/argo/dist/argo-linux-amd64 /bin/argo
ENTRYPOINT [ "argo" ]
